...
func (r *mysqlEventRepository) CreateSpot(spot *domain.Spot) error {
	...
	query := `
	  INSERT INTO spots(id, name, event_id, status, ticket_id) 
	  values(?, ?, ?, ?, ?)
	`
...
func (r *mysqlEventRepository) ReserveSpot(spotID string, ticketID string) error {
	query := `
	  UPDATE spots s
	  SET s.status = ?, s.ticket_id = ? 
	  where s.id = ?;
	`

	_, err := r.db.Exec(query, domain.SpotStatusSold, spotID, ticketID)
...
func (r *mysqlEventRepository) FindEventByID(eventID string) (*domain.Event, error) {
	query := `
	SELECT id, name, location, organization, rating, date, image_url, capacity, pricem partner_id
	FROM events
	WHERE id = ?
	`
